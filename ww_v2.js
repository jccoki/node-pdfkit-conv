...
//var start_row = 306
var excel_file = 'WW\ 2016\ Overall\ Individual\ Adviser\ Report\ Data\ v2.9.xlsx'
var input_sheet = 'Individual Report';

var target_output_dir = './output'
var licensee_logo = 'RI Advice logo.jpg'

// report theme
var header_fill_style= '#f78429'
...
//    doc.path('M 0 '+ (opts.page.height*0.33) +' l 0 -' + header_band_height + ' l ' + opts.page.width + ' 0 l 0 ' + header_band_height + ' l -'+ opts.page.width +' 0').fillAndStroke("#c95109");
    ...
    // front page logos
    doc.image('resources/logos/' + licensee_logo, (opts.page.width*0.40), (opts.page.height*0.2125), {'scale': 0.325})
//    doc.image('resources/ww-frontpage-bg.png', (opts.page.width*0.175), (opts.page.height*0.25), {'scale': 0.45})
    doc.image('resources/ww-frontpage-bg.png', 0, (opts.page.height*0.265), {'width': doc.page.width, 'height': opts.page.height*0.325})
   ...
   //doc.flushPages()
    ...
    for(var i=0; i<thead_heading.length; i++){
       ...
       doc.fillAndStroke(header_fill_style, '#000')
       doc.fillColor('#000')
       doc.text(thead_heading[i].data, curr_x, (curr_y + padding), {'align': 'center', 'width': (totalWidth*thead_heading[i].width)})
    ...
    for(i=0; i<TopExpectations.length; i++){
      ...
      writable_width = (totalWidth*thead_heading[1].width) - 5
      ...
      // very special caser
      if(TopExpectations[i].answer.toLowerCase() == 'keeps me up to date with changes in the investment and regulatory environment'){
    ...
    for(var i=0; i<thead_heading.length; i++){
      // @TODO refactor!
      switch(licensee_name.toLowerCase()){
        case 'ri advice':
          if(i == 2){
            padding = lineHeight;
          }else{
...
//--
    ...
    for(var i=0; i<thead_heading.length; i++){
      ...
      doc.fillColor('#000')
      doc.text(thead_heading[i].data, curr_x, (curr_y + padding), {'align': 'center', 'width': (totalWidth*thead_heading[i].width) - 5})
...
//      question = worksheet[ XLSX.utils.encode_cell({'c': i, 'r': 1}) ].v.replace(' (Answer)', '').replace(' (Text)', '')
      ...
      curr_y = doc.y + lineHeight
      doc.font(opts.font.weight.normal)
      doc.image('resources/params/' + q_key + '.png', curr_x, curr_y, {'scale': 0.1275})
...
//      question = worksheet[ XLSX.utils.encode_cell({'c': i, 'r': 1}) ].v.replace(' (Answer)', '')
      ...
      lines = lines_func(doc_width, (totalWidth - 10))
...
//      question = worksheet[ XLSX.utils.encode_cell({'c': i, 'r': 1}) ].v.replace(' (Answer)', '')
      ...
      // dynamically allocation of table cell height
      doc_width = doc.widthOfString(answer)
      lines = lines_func(doc_width, (totalWidth - 5))
      box_height = doc.currentLineHeight() * (lines + 2)
      doc.rect(curr_x, curr_y, totalWidth, box_height)
      doc.fillAndStroke('#dedede', '#000')
      doc.fillColor('#000')
...
//    question = worksheet[ XLSX.utils.encode_cell({'c': 94, 'r': 1}) ].v.replace(' (Text)', '')
    ...
    for(var i = 0; i < scoreVal.length; i++){
      ...
      // draw the self rating
      curr_x = curr_x + (totalWidth*thead_heading[0].width)
      doc.text(scoreVal[i].answer, (curr_x + 2.5), (curr_y + lineHeight), {'width': (totalWidth*thead_heading[1].width) - 5, 'align': 'center'});
